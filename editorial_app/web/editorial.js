...
function workflowLabel(value) { return {IMPORTED:"Importada",EDITING:"Editada",REVIEW:"Revista",VALIDATED:"Aprovada",PUBLISHED:"Publicada"}[value] || value; }
function workflowClass(value) { return {IMPORTED:"workflow-imported",EDITING:"workflow-editing",REVIEW:"workflow-review",VALIDATED:"workflow-validated",PUBLISHED:"workflow-published"}[value] || "workflow-imported"; }
...
async function loadOverview() {
  ...
  $("#overview").textContent = `${n(run.entry_count)} entradas · ${n(workflow.EDITING)} em edição · ${n(workflow.REVIEW)} em revisão · ${n(workflow.VALIDATED)} validadas`;
  populateUsers(overviewData.users);
  const counts = overviewData.filter_counts;
  countedSelect("#resource-filter", counts.resource, "Dicionário e Vocabulário", run.entry_count, resourceLabel);
  countedSelect("#workflow-filter", completeCounts(counts.workflow, ["IMPORTED","EDITING","REVIEW","VALIDATED","PUBLISHED"]), "Todos", run.entry_count, workflowLabel);
  countedSelect("#source-status-filter", counts.editorial_statuses, "Todos", run.entry_count);
  countedSelect("#grammar-filter", counts.grammar, "Todas", run.entry_count);
  countedSelect("#domain-filter", counts.domains, "Todos", run.entry_count);
  countedSelect("#severity-filter", completeCounts(counts.severity, ["error","warning","info"]), "Todos", run.entry_count, value => ({error:"Erros",warning:"Avisos",info:"Informação"}[value]));
}
function populateUsers(users) {
  ...
  select.onchange = () => { localStorage.setItem("acl-editorial-user", select.value); updateRoleIndicator(); if ($("#governance-panel").classList.contains("active")) loadControlledValues(); if ($("#publication-panel").classList.contains("active")) loadPublicationEntries(); };
  updateRoleIndicator();
}
function updateRoleIndicator() { const role = currentUser()?.role || ""; const node = $("#role-indicator"); node.textContent = roleLabel(role); node.className = `role-indicator role-${role}`; document.body.dataset.editorialRole = role; $("#add-controlled").disabled = !["reviewer","approver","administrator"].includes(role); }
function countedSelect(selector, values = [], first, total, label = value => value) { const select = $(selector); const previous = select.value; select.innerHTML = `<option value="">${h(first)} (${n(total)})</option>` + values.map(item => `<option value="${h(item.value)}">${h(label(item.value))} (${n(item.count)})</option>`).join(""); select.value = previous; }
...
function renderPublicEntry(item) {
  ...
  $("#edit-entry").addEventListener("click", () => renderEditEntry(item)); bindEntryActions(item);
}
...
function renderWorkflow(item) { const targets = {IMPORTED:["EDITING"],EDITING:["REVIEW"],REVIEW:["EDITING","VALIDATED"],VALIDATED:["EDITING"],PUBLISHED:["EDITING"]}[item.workflow_status] || []; return `<div class="workflow-actions"><strong>Workflow:</strong>${targets.map(target => `<button class="secondary" type="button" data-workflow="${target}">${h(target === "REVIEW" ? "Submeter para revisão" : workflowLabel(target))}</button>`).join("")}</div>`; }
...
// Publicação seletiva
async function loadPublicationEntries(){try{const data=await api("/api/editorial/publication-entries?limit=500");publicationItems=data.items;const canSelect=["approver","administrator"].includes(currentUser()?.role);$("#select-visible").disabled=!canSelect;$("#unselect-visible").disabled=!canSelect;$("#publication-selection-summary").textContent=`${n(data.selected)} selecionadas de ${n(data.total)} entradas validadas`;$("#publication-entries").innerHTML=data.items.map(item=>`<label class="publication-entry"><input type="checkbox" data-publication-id="${h(item.public_id)}"${item.selected ? " checked" : ""}${canSelect ? "" : " disabled"}><span><strong>${h(item.lemma)}</strong><small>${h(resourceLabel(item.resource))} · ${h(item.grammatical_info || "sem classe")}</small></span></label>`).join("")||"<p>Não existem entradas validadas à espera de publicação.</p>";document.querySelectorAll("[data-publication-id]").forEach(node=>node.addEventListener("change",()=>setPublicationSelection([node.dataset.publicationId],node.checked)));}catch(error){$("#publication-selection-summary").textContent=error.message;}}
...
$("#validate").addEventListener("click",async()=>{const button=$("#validate");button.disabled=true;$("#validation-status").textContent="A validar o corpus…";try{renderValidation(await api("/api/editorial/validate",{method:"POST",body:"{}"}));await loadOverview();}catch(error){toast(error.message,true);}finally{button.disabled=false;}});
function renderValidation(result){$("#validation-status").textContent=`${result.valid ? "Válido" : "Com erros bloqueantes"} · ${n(result.blocking_errors ?? result.errors)} erros · ${n(result.warnings)} avisos`;}
$("#prepare-release").addEventListener("click",async()=>{try{await api("/api/editorial/releases/prepare",{method:"POST",body:JSON.stringify({actor:actor(),description:$("#release-description").value})});publicationTimer=setInterval(loadPublication,1000);loadPublication();}catch(error){toast(error.message,true);}});
async function loadPublication(){try{const [job,validation]=await Promise.all([api("/api/editorial/publish/status"),api("/api/editorial/validation")]);renderValidation(validation);$("#publication-status").textContent=job.message||"Nenhuma operação em curso.";$("#prepare-release").disabled=job.state==="running";renderReleases(job.releases||[],job.active_release);await loadPublicationEntries();if(job.state!=="running"&&publicationTimer){clearInterval(publicationTimer);publicationTimer=null;if(job.state==="succeeded")toast(job.message);if(job.state==="failed")toast(job.message,true);}}catch(error){$("#publication-status").textContent=error.message;}}
function renderReleases(releases,active){$("#releases").innerHTML=`<table><thead><tr><th>Versão</th><th>Estado</th><th>Responsáveis</th><th>Data</th><th>Ações</th></tr></thead><tbody>${releases.map(release=>{const actions=[];if(release.state==="candidate")actions.push(`<button data-release-action="approve" data-release="${h(release.release_id)}">Aprovar</button>`);if(release.state==="approved")actions.push(`<button data-release-action="publish" data-release="${h(release.release_id)}">Publicar</button>`);if(release.state==="archived")actions.push(`<button class="secondary" data-release-action="rollback" data-release="${h(release.release_id)}">Reverter</button>`);return `<tr><td><strong>${h(release.release_id)}</strong>${release.release_id===active?'<br><span class="active-release">Ativa</span>':""}<br><small>${h(release.description||"")}</small></td><td>${h(release.state)}</td><td>${h(release.prepared_by||"—")}<br>${h(release.approved_by||"—")}</td><td>${h(formatDate(release.created_at))}</td><td class="table-actions">${actions.join(" ")}</td></tr>`;}).join("")}</tbody></table>`;document.querySelectorAll("[data-release-action]").forEach(node=>node.addEventListener("click",()=>releaseAction(node.dataset.release,node.dataset.releaseAction)));}
async function releaseAction(id,action){const labels={approve:"aprovar",publish:"publicar",rollback:"reverter para"};if(!confirm(`Confirma que pretende ${labels[action]} ${id}?`))return;try{await api(`/api/editorial/releases/${encodeURIComponent(id)}/${action}`,{method:"POST",body:JSON.stringify({actor:actor(),comment:`${labels[action]} ${id}`})});if(action!=="approve")publicationTimer=setInterval(loadPublication,1000);loadPublication();}catch(error){toast(error.message,true);}}
